...
Two versions are provided (MuonAdamW), for single GPU and distributed.
...
def _make_adamw_step_fused():
    """Factory: returns a compiled adamw_step_fused. Called once at optimizer init."""
    @torch.compile(dynamic=False, fullgraph=True)
    def adamw_step_fused(
        p: Tensor,              # (vocab, 768) - parameter tensor
        grad: Tensor,           # (vocab, 768) - gradient, same shape as p
        exp_avg: Tensor,        # (vocab, 768) - first moment, same shape as p
        exp_avg_sq: Tensor,     # (vocab, 768) - second moment, same shape as p
...
def _make_muon_step_fused(compute_dtype: torch.dtype):
    """Factory: returns a compiled muon_step_fused with compute_dtype baked in via closure.
    Called once at optimizer init — torch.compile traces the closure value on first step.
    """
    @torch.compile(dynamic=False, fullgraph=True)
    def muon_step_fused(
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    Arguments:
        ...
        compute_dtype: dtype for Muon's polar express bf16 cast (default: float32).
            Pass torch.bfloat16 when training in bf16.
    """
    def __init__(self, param_groups: list[dict], compute_dtype: torch.dtype = torch.float32):
        super().__init__(param_groups, defaults={})
        self._adamw_step = _make_adamw_step_fused()
        self._muon_step = _make_muon_step_fused(compute_dtype)
